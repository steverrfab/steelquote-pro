...
export function Toggle({ value, onChange, large }) {
  ...
  return (
    <div
      ...
      style={{ width: w, height: h, background: value ? '#7c3aed' : '#1e2532', borderRadius: h, cursor: 'pointer', position: 'relative', transition: 'background 0.2s', flexShrink: 0 }}
    >
      <div style={{ position: 'absolute', top: (h - d) / 2, left: value ? w - d - (h - d) / 2 : (h - d) / 2, width: d, height: d, background: '#edf0f4', borderRadius: '50%', transition: 'left 0.2s' }} />
    </div>
  )
}
...
export function Btn({ children, onClick, color }) {
  return (
    <button onClick={onClick} style={{ background: '#1a2030', border: `1px solid ${color || '#2d3340'}`, borderRadius: 6, color: color || '#c8cdd6', padding: '8px 14px', fontSize: 10, cursor: 'pointer', fontFamily: 'inherit', letterSpacing: 1, textTransform: 'uppercase' }}>
      {children}
    </button>
  )
}
